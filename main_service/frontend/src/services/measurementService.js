...
export const createMeasurementDraft = async (serial_a, serial_b, product_id, status) => {
    const response = await apiClient(API_BASE_URL_MEASUREMENT + '/draft', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({ serial_a, serial_b, product_id, status })
    });

    if (!response.ok) {
        // ✅ เพิ่มการดึงข้อมูล Error มาดูว่าหลังบ้านบ่นอะไร
        const errorData = await response.json();
        console.error("Backend Error Details:", errorData);
        throw new Error(errorData.message || "Create measurements draft failed");
    }

    const result = await response.json();
    // ✅ Backend ของคุณหุ้ม data ไว้ใน key 'data'
    return result; 
};
